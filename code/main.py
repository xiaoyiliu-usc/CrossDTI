...
def main(args):
    ...
    for iteration in range(args.iterations):
        ...
        model = MVP(num_classes=args.class_num, in_feats=n_feats, hidden_feats=args.hidden_feats,
                    ...
                    protein_cnn_dim=args.protein_cnn_dim, protein_rnn_dim=args.protein_rnn_dim,
        ...
        stopper = EarlyStopping(mode='higher', filename=f'{args.output}/net_{iteration}.pkl', patience=30)
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                all_prediction_probabilities.extend(pred_prob.cpu().numpy())

        prediction_details = pd.DataFrame({
            'true_label': all_true_labels,
            'predicted_label': all_predicted_labels,
            'prediction_probability': all_prediction_probabilities
        })

        prediction_details_file = f'{args.output}/prediction_details_iteration_{iteration}.csv'
        prediction_details.to_csv(prediction_details_file, index=False)
        ...
        r = test_results.mean()
        print(
            f"test_---acc:{r['acc']}---precision:{r['precision']}---recall:{r['recall']}---f1_score:{r['f1_score']}---auroc:{r['auroc']:.4f}---auprc:{r['auprc']:.4f}")
        mean_results.append([r['acc'], r['precision'], r['recall'], r['f1_score'], r['auroc'], r['auprc']])
        test_mean_results = pd.DataFrame(mean_results,
                                         columns=['acc', 'precision', 'recall', 'f1_score', 'auroc', 'auprc'])
