...
from dgl.nn.pytorch import NNConv, Set2Set  # 修改导入
...
class ProteinCNN(nn.Module):
    ...
    def forward(self, x):
        # x shape: [batch_size, seq_len, protein_Oridim]
...
class MVP(nn.Module):
    def __init__(self, num_classes=1, in_feats=64, hidden_feats=None, num_step_set2set=6,
                 ...
                 protein_cnn_dim=343, protein_rnn_dim=128, protein_vocab_size=21,  # 20个氨基酸+padding
                 ctf_dim=343,
        ...
        num_step_set2set: Set2Set 步数
        num_layer_set2set: Set2Set 层数
        rnn_embed_dim: RNN 嵌入维度
        blstm_dim: 双向 LSTM 的隐藏维度
        blstm_layers: LSTM 层数
        fp_2_dim: 分子指纹的维度
        physchem_dim: 新增的物理化学特征处理的隐藏维度
        ...
        self.vocab = tokens_struct()  # 初始化 SMILES 字符串的编码结构
        if hidden_feats is None:
            hidden_feats = [64, 64]  # 如果没有指定隐藏层特征维度，使用默认值 [64, 64]
        ...
        self.gnn = GNNModule(in_feats, hidden_feats, dropout, num_step_set2set, num_layer_set2set)  # 图神经网络模块
        self.rnn = RNNModule(self.vocab, rnn_embed_dim, blstm_dim, blstm_layers, self.final_hidden_feats, dropout,
                             bidirectional=True, device=device)  # RNN模块
        self.fp_mlp = FPNModule(fp_2_dim, self.final_hidden_feats)  # 分子指纹模块
        ...
        self.protein_cnn = ProteinCNN(
            protein_Oridim=20,  # 20种氨基酸的one-hot编码
            feature_size=200,
        ...
        self.num_views = 6  # SMILES, Graph, FP, PhysChem, Protein
        ...
        dim_k = self.final_hidden_feats * num_heads  # 多头注意力机制中的键维度
        dim_v = self.final_hidden_feats * num_heads  # 多头注意力机制中的值维度
        dim_in = self.final_hidden_feats  # 输入维度
        assert dim_k % num_heads == 0 and dim_v % num_heads == 0, "dim_k and dim_v must be multiple of num_heads"
        self.dim_in = dim_in
        self.dim_k = dim_k
        self.dim_v = dim_v
        self.num_heads = num_heads
        self.linear_q = nn.Linear(dim_in, dim_k, bias=False)  # 线性变换
        self.linear_k = nn.Linear(dim_in, dim_k, bias=False)
        self.linear_v = nn.Linear(dim_in, dim_v, bias=False)
        self._norm_fact = 1 / sqrt(dim_k // num_heads)  # 正则化因子

        # 计算卷积后的输出维度
        # 输入: [batch_size, num_heads, 4, final_hidden_feats]
        # 卷积核: [2, 2]，步长: 1，无填充
        # 输出: [batch_size, num_heads, 3, final_hidden_feats-1]
        conv_output_height = self.num_views - 2 + 1  # = 5
        conv_output_width = self.final_hidden_feats - 2 + 1  # = final_hidden_feats - 1
        ...
        # 添加自注意力层
        # self.self_attention = SelfAttention(self.final_hidden_feats, dropout=dropout).to(device)
        ...
        self.mlp = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(dropout * 1.5),  # 略微增加dropout率
    ...
    def forward(self, smiles, graphs, atom_feats, fp_t, physchem_t, protein_data, ctf_t):
        '''
        protein_data: 字典，包含蛋白质序列信息
        - 'protein_cnn': CNN输入，形状为[batch_size, max_seq_len, 20]
        - 'protein_seq': RNN输入，形状为[batch_size, protein_seq_len]
        - 'protein_seq_len': 序列长度列表
        '''
        # 获取各个模块的特征表示
        ...
        smiles_enhanced = self.cross_attention_smiles_to_graph(
            query=smiles_x,  # 查询来自SMILES
            key=graph_x,  # 键来自图特征
            value=graph_x  # 值也来自图特征
        )
        ...
        in_tensor = torch.cat([
            ...
            protein_enhanced,
            ctf_x
        ], dim=1)  # [batch_size, 6, final_hidden_feats]

        # 原有的注意力机制和卷积处理
        batch, n, dim_in = in_tensor.shape
        assert dim_in == self.dim_in
        nh = self.num_heads
        dk = self.dim_k // nh  # 每个head的dim_k
        dv = self.dim_v // nh  # 每个head的dim_v
        ...
        out = self.conv(att)
        out = out.view(batch_size, -1)  # 卷积操作后展平

        out = self.mlp(out)  # MLP处理
...
class CrossAttention(nn.Module):
    def __init__(self, query_dim, key_dim, value_dim, num_heads=4, dropout=0.1):
        ...
        self.w_k = nn.Linear(key_dim, query_dim, bias=False)  # 投影到query维度
        self.w_v = nn.Linear(value_dim, query_dim, bias=False)  # 投影到query维度
    ...
    def forward(self, query, key, value):
        """
        query: [batch_size, seq_len_q, query_dim] - 作为查询的特征
        key: [batch_size, seq_len_k, key_dim] - 作为键的特征
        value: [batch_size, seq_len_v, value_dim] - 作为值的特征
        """
    ...
    def __init__(self, in_feats=64, hidden_feats=None, dropout=0.2, num_step_set2set=6,
        ...
        for i, hidden_dim in enumerate(hidden_feats):
            ...
            conv_layer = NNConv(
                ...
                aggregator_type='mean'  # 可以使用'mean', 'sum', 'max'
            )
        ...
        self.fc = nn.Sequential(
            ...
            nn.BatchNorm1d(hidden_feats[-1] * 2),  # 添加BatchNorm
            nn.ReLU(),
            nn.Dropout(p=dropout),
...
class PhysChemModule(nn.Module):
    ...
    def forward(self, x):
        # 适应 BatchNorm1d，需要 [batch_size, features] 的形状
        orig_x = x
        if len(x.shape) == 1:  # 如果是1D张量，添加批次维度
            x = x.unsqueeze(0)
