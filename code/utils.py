...
def accuracy(y_true, y_pred):
    """
    y_true: 真实标签，形状为 [n_samples] 的数组，值为0或1
    y_pred: 预测标签，形状为 [n_samples] 的数组，值为0或1
    """
    # 确保输入是numpy数组
...
def evaluate(label, output):
    ...
    # 将预测值通过sigmoid转换为概率，然后二值化为0或1
    preds_prob = 1 / (1 + np.exp(-zs))  # sigmoid函数
    ...
    # 计算精确率、召回率和F1分数
    precision = precision_score(ts, preds, average='binary', zero_division=0)
    recall = recall_score(ts, preds, average='binary', zero_division=0)
    f1score = f1_score(ts, preds, average='binary', zero_division=0)
    try:
        auroc = roc_auc_score(ts, preds_prob)
    except:
        auroc = 0.0
    try:
        auprc = average_precision_score(ts, preds_prob)
    except:
        auprc = 0.0

    return acc, precision, recall, f1score, auroc, auprc, acc_weight
